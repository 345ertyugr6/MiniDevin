"""
Planner & Coder Module
Breaks down tasks into steps and generates code
"""

import asyncio
import json
import logging
from typing import Dict, Any, List
from .llm_client import LLMClient


LOGGER = logging.getLogger(__name__)


class Planner:
    def __init__(self, llm_client: LLMClient):
        self.llm_client = llm_client
        self.plans = []
    
    async def create_plan(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """
        Create a step-by-step plan for the given task
        
        Args:
            task: Task dictionary from PromptInterface
            
        Returns:
            Plan dictionary with steps
        """
        prompt = self._build_planning_prompt(task)
        LOGGER.debug("Requesting plan for task: %s", task.get("raw_input"))
        response = await self.llm_client.generate(prompt)
        LOGGER.debug("Planner raw response: %s", response.strip()[:2000])

        if self._is_error_response(response):
            plan = {
                "task": task,
                "steps": [],
                "status": "failed",
                "error": response.strip()
            }
            LOGGER.debug("Planner marked plan as failed: %s", plan.get("error"))
        else:
            plan = {
                "task": task,
                "steps": self._parse_steps(response),
                "status": "pending"
            }
            LOGGER.debug("Planner produced %d step(s)", len(plan["steps"]))

        self.plans.append(plan)
        return plan

    def _build_planning_prompt(self, task: Dict[str, Any]) -> str:
        """Build prompt for planning"""
        return f"""You are a software development planner. Break down the following task into clear, executable steps.

Task: {task['raw_input']}
Task Type: {task['task_type']}

Guidelines:
- Focus on the concrete coding and analysis work required inside the current repository.
- Assume Python and basic tooling are already available—do NOT include steps about checking installations or setting up environments.
- Limit the plan to the essential actions (no more than 8 steps) that directly move the task forward.
- Each step should be specific, testable, and ready for the coding agent to execute.

Provide the steps as a numbered list (1., 2., 3., etc.)."""

    def _parse_steps(self, response: str) -> List[Dict[str, Any]]:
        """Parse steps from LLM response"""
        steps = []
        lines = response.strip().split('\n')
        
        for line in lines:
            line = line.strip()
            if line and (line[0].isdigit() or line.startswith('-') or line.startswith('*')):
                step_text = line.lstrip('0123456789.-*) ').strip()
                if step_text:
                    steps.append({
                        "description": step_text,
                        "status": "pending",
                        "code": None,
                        "result": None
                    })

        if steps:
            LOGGER.debug("Parsed %d structured step(s) from planner response", len(steps))
            return steps

        LOGGER.debug("Planner response did not contain numbered steps; returning fallback entry")
        return [{"description": response, "status": "pending", "code": None, "result": None}]

    @staticmethod
    def _is_error_response(response: str) -> bool:
        """Detect whether the LLM response indicates an error instead of plan steps."""

        if not response:
            return False

        normalized = response.strip().lower()
        return normalized.startswith("error:") or normalized.startswith("error generating response:")


class Coder:
    def __init__(self, llm_client: LLMClient):
        self.llm_client = llm_client
    
    async def generate_code(self, step: Dict[str, Any], context: Dict[str, Any] = None) -> str:
        """
        Generate code for a specific step
        
        Args:
            step: Step dictionary from plan
            context: Additional context (previous steps, errors, etc.)
            
        Returns:
            Generated code as string
        """
        LOGGER.debug("Generating code for step: %s", step.get("description"))
        if context:
            LOGGER.debug("Coder received context keys: %s", list(context.keys()))

        prompt = self._build_coding_prompt(step, context)
        code = await self.llm_client.generate(prompt)

        code = self._extract_code_block(code)
        LOGGER.debug("Generated code length: %d characters", len(code))
        return code
    
    def _build_coding_prompt(self, step: Dict[str, Any], context: Dict[str, Any] = None) -> str:
        """Build prompt for code generation"""
        base_prompt = f"""You are an expert Python programmer. Generate clean, working Python code for the following task.

Task: {step['description']}

Requirements:
- Write complete, executable Python code
- Include necessary imports
- Add minimal error handling
- Keep it simple and efficient
- Output ONLY the code, no explanations"""

        if context and context.get("previous_error"):
            base_prompt += f"\n\nPrevious attempt failed with error:\n{context['previous_error']}\n\nFix the error and provide corrected code."

        if context and context.get("installed_packages"):
            installed = ", ".join(context["installed_packages"])
            base_prompt += (
                "\n\nThe following packages have already been installed in this environment: "
                f"{installed}. Avoid reinstalling them unnecessarily."
            )

        if context and context.get("search_results"):
            base_prompt += f"\n\nRelevant information from search:\n{context['search_results']}\n\nUse this information to improve your solution."

        return base_prompt
    
    def _extract_code_block(self, response: str) -> str:
        """Extract code from markdown code blocks or raw response"""
        lines = response.split('\n')
        code_lines = []
        in_code_block = False
        
        for line in lines:
            if line.strip().startswith('```'):
                in_code_block = not in_code_block
                continue
            if in_code_block or (not any(line.strip().startswith(x) for x in ['#', 'Here', 'This', 'The', 'Note']) and line.strip()):
                code_lines.append(line)
        
        code = '\n'.join(code_lines).strip()
        
        if not code or len(code) < 10:
            code = response.strip()
        
        return code
